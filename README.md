...
- `8GB` 장비는 `E2B only, best-effort`로 보는 편이 안전합니다.
...
- `tmp/pdfs/`: PDF 검토용 contact sheet 이미지
...
- 검토 이미지: `tmp/pdfs/gemma4-hands-on-prep-contact-*.png`
...
- 행사 안내 문구는 **참가자 입장에서 바로 따라 할 수 있게** 유지합니다.
- 모델/도구 추천은 **안정성 우선**으로 정리합니다.
- 운영체제별 차이와 Windows의 `PowerShell` / `WSL2` 구분을 명확히 적습니다.
- 상대적으로 고급 경로인 `OpenCode`, `Hermes Agent`, `llama.cpp`는 기본 경로와 분리해 설명합니다.
