...
- `starter/`: 참석자용 시작 코드입니다. `translate`만 바로 실행 가능하고, 핸즈온 중 `demo/scenarios.py`에 시나리오를 하나씩 추가합니다.
- `final/`: 강사용/정답용 최종 완성본입니다. 모든 시나리오가 등록되어 있습니다.
- `HANDS_ON.md`: starter에서 final까지 단계별로 구현하는 실습 문서입니다.
...
참석자용 시작 코드는 `workshop/starter` 폴더에서 실행합니다.
...
PowerShell에서 이 저장소의 `workshop/starter` 폴더로 이동한 뒤 실행합니다.
...
단계별로 시나리오를 추가하면서 진행하는 핸즈온 운영안은 [HANDS_ON.md](HANDS_ON.md)를 참고하세요.
...
`story_revision`은 `story_review_selection`의 선정 보고서와 `short_story_writing`의 원본 단편 원고를 함께 읽고, 리뷰 내용을 반영한 개정 원고를 `revised_stories/`에 저장합니다. 선정 보고서의 `선정작` 섹션에서 작품 파일명을 먼저 추출하고 중복을 제거한 뒤 agent별로 하나씩 배정하므로, 같은 작품이 여러 번 개정되지 않습니다.

## 새 시나리오 추가

핸즈온에서는 `starter/demo/scenarios.py`를 수정합니다. 전체 단계별 코드는 [HANDS_ON.md](HANDS_ON.md)에 정리되어 있습니다.
