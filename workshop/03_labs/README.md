...
- [진행 방식](#진행-방식)
- [Step 0. LM Studio 앱에서 코드와 이력서 직접 생성](#step-0-lm-studio-앱에서-코드와-이력서-직접-생성)
...
```text
LM Studio 앱에서 Flutter Todo 앱 코드와 이력서 1개 직접 생성
...
## Step 0. LM Studio 앱에서 코드와 이력서 직접 생성
...
먼저 system instruction 없이 LM Studio 앱의 user message 입력창에 아래 프롬프트를 그대로 넣습니다.

```text
Translate the following sentence into Korean.
Output only the translated sentence.

Gemma 4 is a family of models released by Google DeepMind.
```

확인할 것:

- 한국어 번역문만 출력되는지
- 설명, 목록, 코드블록 없이 한 문장으로 나오는지
- `Gemma 4`, `Google DeepMind` 같은 고유명사가 자연스럽게 유지되는지

### 코드 생성 워밍업

이어서 같은 방식으로 아래 프롬프트를 넣어 간단한 Flutter 앱 코드를 생성합니다.

```text
You are a careful programmer.

Create a simple Flutter Todo app.

Requirements:
- Output Dart code only.
- Do not use Markdown code fences.
- Put everything in a single main.dart file.
- Use Material 3.
- The app title is "Gemma Todo".
- Users can add a todo item from a text field.
- Users can mark a todo item as done.
- Users can delete a todo item.
- Keep state locally with StatefulWidget. Do not use external packages.
- Include clean, readable widget structure and comments only where helpful.
```

확인할 것:

- 설명 없이 Dart 코드만 출력되는지
- `main.dart` 하나로 붙여 넣어 실행할 수 있는지
- Todo 추가, 완료 표시, 삭제 기능이 모두 있는지
- 외부 패키지 없이 `StatefulWidget`으로 동작하는지
...
도서 출판사에서 일할 "소설 기획편집자" 포지션에 지원하는 가상의 한국인 후보자 이력서 1개를 작성하세요.
...
- 일반 IT 기획자가 아니라 소설/문학 편집과 출판 기획을 함께 다루는 직무에 맞게 작성합니다.
- 후보자의 경력, 담당 장르, 출판사 유형, 프로젝트, 성과, 사용 도구가 구체적으로 드러나야 합니다.

반드시 아래 구조를 따르세요.

# [가상의 한국인 이름] - 도서 출판사 소설 기획편집자 이력서
## 프로필
## 핵심 역량
## 경력
## 주요 기획 프로젝트
...
목표는 콘텐츠 생성 프롬프트와 코드 생성 프롬프트의 차이를 비교하는 것입니다. Flutter Todo 앱 코드와 이력서 1개를 만드는 Step 0은 user message만으로 충분하지만, `scenarios.py`에 붙여 넣을 코드 초안을 만들 때는 system instruction으로 역할과 출력 형식을 고정하는 편이 안정적입니다.
...
- 시나리오 이름: resume
- agent 수 기본값: 10
- 각 agent는 도서 출판사 소설 기획편집자 지원자 이력서 1개를 생성합니다.
- 결과는 한국어 Markdown입니다.
- 저장 폴더는 resumes 입니다.
...
`Invoke-RestMethod`가 실패하면 LM Studio Local Server가 꺼져 있거나 포트가 다른 상태입니다. 최신 `run.ps1`은 `Server`와 `Model`을 먼저 출력하고, 서버나 모델을 확인하지 못하면 agent 창 10개를 띄우기 전에 멈춥니다. 여러 창이 뜬 뒤 모두 같은 `[ERROR]`를 보이면 한 창의 에러 첫 줄을 확인하고, 모델/포트 오류라면 `--port` 또는 `--model`을 명시합니다.
...
| `resume` | 소설 기획편집자 이력서 생성 | Step 3 | `resumes/` |
...
| `interview_dialogue` | 이력서 기반 실제 멀티턴 면접 실행 | Step 6 | `interview_dialogues/` |
...
Step 3부터 제공되는 코드는 [01_starter/demo/scenarios.py](../01_starter/demo/scenarios.py)에 두 위치로 나누어 붙입니다. 아래 줄 번호는 새 `01_starter` 기준입니다. 실습하면서 코드가 늘어나면 줄 번호는 조금씩 밀릴 수 있으므로, 줄 번호보다 "검색할 문구"를 찾는 방식이 더 안전합니다.

| 붙일 내용 | 현재 starter 기준 위치 | 검색할 문구 | 붙여 넣는 위치 |
| --- | --- | --- | --- |
| `make_xxx_agents()`, `XXX_SYSTEM`, `XXX_PLAN` 같은 구현 코드 | 211줄 근처 | `Paste scenario implementation snippets` | 첫 번째 `# TODO Step 3~13:` 블록 아래, `def translate_card`보다 위 |
| `"scenario_name": {...}` registry 등록 코드 | 531줄 근처 | `Paste each registry entry` | `SCENARIOS = {` 안쪽, `code` 등록 블록 뒤의 두 번째 `# TODO Step 3~13:` 블록 아래 |

Step 3을 예로 들면 아래처럼 나누어 붙입니다.

```text
1. make_resume_agents(), RESUME_SYSTEM, RESUME_PLAN
   -> 211줄 근처 첫 번째 TODO 영역에 붙여 넣기

2. "resume": { ... }
   -> 531줄 근처 SCENARIOS dict 안쪽 두 번째 TODO 영역에 붙여 넣기
```

이후 Step 4, Step 5도 같은 방식입니다. 함수와 prompt 상수는 첫 번째 TODO 영역에 계속 아래로 추가하고, `SCENARIOS["..."]`에 해당하는 dict 항목은 두 번째 TODO 영역에 계속 아래로 추가합니다. 줄 번호는 새 starter 기준이며, 코드를 붙여 넣을수록 두 번째 TODO 영역은 590줄대처럼 아래로 밀릴 수 있습니다.

주의할 점:

- registry 항목은 `SCENARIOS = { ... }` 딕셔너리 안쪽에 있어야 합니다.
- registry 항목 끝에는 쉼표를 붙입니다.
- `make_xxx_agents`, `XXX_SYSTEM`, `XXX_PLAN`, `render_card` 이름이 registry에서 참조하는 이름과 정확히 같아야 합니다.
- 새 card renderer 함수를 직접 추가하는 경우에도 첫 번째 TODO 영역에 붙입니다. 현재 starter에는 이 문서에서 쓰는 기본 renderer들이 미리 들어 있습니다.
...
붙여넣기 위치: `01_starter/demo/scenarios.py`의 첫 번째 TODO 영역, 현재 211줄 근처입니다. `Paste scenario implementation snippets`를 검색하면 됩니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. 바로 위의 `make_resume_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `RESUME_SYSTEM` 아래에 이어서 붙입니다.
...
붙여넣기 위치: `SCENARIOS = { ... }` 안쪽의 두 번째 TODO 영역, 새 starter 기준 531줄 근처입니다. 이미 앞 코드를 붙였다면 더 아래로 밀릴 수 있으므로 `Paste each registry entry`를 검색하면 됩니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. Step 3에서 붙인 `RESUME_PLAN` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_interview_review_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `INTERVIEW_REVIEW_SYSTEM` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. Step 3에서 등록한 `"resume": { ... }` 블록 뒤에 이어서 붙입니다.
...
```powershell
.\run.ps1 --scenario interview_review --topic "도서 출판사 소설 기획편집자" --tasks 10
```
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. Step 4 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_hiring_decision_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `HIRING_DECISION_SYSTEM` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. Step 4에서 등록한 `"interview_review": { ... }` 블록 뒤에 이어서 붙입니다.
...
```powershell
.\run.ps1 --scenario hiring_decision --topic "도서 출판사 소설 기획편집자" --hires 2
```
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. Step 5 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_marketer_resume_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `MARKETER_RESUME_SYSTEM` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. `"hiring_decision": { ... }` 블록 뒤에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. 마케터 이력서 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_marketer_interview_review_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `MARKETER_INTERVIEW_REVIEW_SYSTEM` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. `"marketer_resume": { ... }` 블록 뒤에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. 마케터 면접 리뷰 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_marketer_hiring_decision_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `MARKETER_HIRING_DECISION_SYSTEM` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. `"marketer_interview_review": { ... }` 블록 뒤에 이어서 붙입니다.
...
목표는 같은 `resumes/` 입력을 다른 방식으로 활용하는 파생 시나리오를 추가하는 것입니다. 이 단계는 대화록을 한 번에 쓰게 하지 않고, specialist가 질문 생성과 답변 생성을 여러 번 번갈아 호출해 실제 멀티턴 면접처럼 실행합니다.

agent factory를 추가합니다.

붙여넣기 위치: 첫 번째 TODO 영역입니다. 마케터 채용 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_interview_dialogue_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: `INTERVIEW_DIALOGUE_PLAN`은 첫 번째 TODO 영역, `"interview_dialogue": { ... }` registry는 두 번째 TODO 영역입니다.
...
```powershell
.\run.ps1 --scenario interview_dialogue --topic "도서 출판사 소설 기획편집자" --tasks 3
```

핵심 설명:

- `execution_mode: "interview_multiturn"`이 specialist에게 실제 멀티턴 실행을 지시합니다.
- `interview_turns: 4`는 한 후보자당 질문 4번, 답변 4번을 번갈아 생성한다는 뜻입니다.
- 최종 Markdown에는 각 Turn의 질문/답변과 면접관 최종 평가가 함께 저장됩니다.
- 후보자 1명마다 여러 번 호출되므로 처음에는 `--tasks 3`으로 확인하는 편이 좋습니다.

대화록 기반 최종 채용 결정은 `hiring_decision`과 같은 구조를 사용하되 입력 폴더만 바꿉니다.

붙여넣기 위치: 아래 factory, system prompt, plan prompt는 첫 번째 TODO 영역입니다. 대화록 시나리오 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. `"interview_dialogue": { ... }` 블록 뒤에 이어서 붙입니다.
...
```powershell
.\run.ps1 --scenario hiring_decision_from_dialogue --topic "도서 출판사 소설 기획편집자" --hires 2
```
...
붙여넣기 위치: 아래 factory, system prompt, plan prompt, renderer는 첫 번째 TODO 영역입니다. registry만 두 번째 TODO 영역에 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `NOVEL_WRITING_PLAN` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. 선택 확장이므로 필수 흐름 registry 뒤나 원하는 위치에 붙여도 됩니다.
...
```powershell
.\run.ps1 --scenario novel_writing --topic "도시 미스터리 장편소설 기획" --tasks 10
```
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. 앞 단계 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: `SHORT_STORY_WRITING_SYSTEM`, `SHORT_STORY_WRITING_PLAN`은 첫 번째 TODO 영역입니다. `"short_story_writing": { ... }` registry는 두 번째 TODO 영역입니다.
...
```powershell
.\run.ps1 --scenario short_story_writing --topic "문예지 신인상 투고용 도시 미스터리 단편" --tasks 10
```
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. Step 7 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: `STORY_REVIEW_SELECTION_SYSTEM`, `STORY_REVIEW_SELECTION_PLAN`은 첫 번째 TODO 영역입니다. `"story_review_selection": { ... }` registry는 두 번째 TODO 영역입니다.
...
```powershell
.\run.ps1 --scenario story_review_selection --topic "문예지 신인상 투고용 도시 미스터리 단편" --select 3 --tasks 3
```
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. Step 8 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_publication_offer_email_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. `"story_review_selection": { ... }` 블록 뒤에 이어서 붙입니다.
...
```powershell
.\run.ps1 --scenario publication_offer_email
```
...
붙여넣기 위치: 아래 factory, system prompt, plan prompt는 첫 번째 TODO 영역입니다. `"contract_negotiation": { ... }` registry는 두 번째 TODO 영역입니다.
...
```powershell
.\run.ps1 --scenario contract_negotiation
```
...
붙여넣기 위치: 아래 factory, system prompt, plan prompt는 첫 번째 TODO 영역입니다. `"contract_draft": { ... }` registry는 두 번째 TODO 영역입니다.
...
```powershell
.\run.ps1 --scenario contract_draft
```
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. Step 11 구현 코드 아래에 이어서 붙입니다.
...
붙여넣기 위치: 첫 번째 TODO 영역입니다. `make_story_revision_agents()` 아래에 이어서 붙입니다.
...
붙여넣기 위치: 두 번째 TODO 영역입니다. `"contract_draft": { ... }` 블록 뒤에 이어서 붙입니다.
...
```powershell
.\run.ps1 --scenario story_revision
```
...
붙여넣기 위치: 아래 factory, system prompt, plan prompt는 첫 번째 TODO 영역입니다. `"marketing_copy": { ... }` registry는 두 번째 TODO 영역입니다.
...
```powershell
.\run.ps1 --scenario marketing_copy --tasks 10
```
