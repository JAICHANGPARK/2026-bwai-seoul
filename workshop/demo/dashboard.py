...
def fetch_server_metrics(server_url: str, enabled: bool = False) -> dict:
...
def build_event_title() -> Panel:
    ...
    return Panel(
        Group(
            ...
            Align.center(Text(GEMMA4_ART, style="bold bright_blue")),
        ),
...
def build_hero(metrics: dict[str, dict], server_metrics: dict = None) -> Panel:
    ...
    Sums per-agent t/s for live throughput.
    """
    sum_tps = 0.0
    running = 0
    done = 0
    ...
    big_text = Text(big_art, style="bold bright_blue", justify="left")
    ...
    content = Group(
        ...
        Align.center(Text("tokens / sec", style="bold bright_blue")),
        Text(""),
...
def build_dashboard(agents: list[dict], metrics: dict[str, dict], server_metrics: dict = None) -> Layout:
    ...
    layout["left"].split_column(
        Layout(name="event_title", size=9),
        Layout(name="hero"),
    )
    ...
    layout["event_title"].update(build_event_title())
...
def main():
    ...
    parser.add_argument("--server-metrics", choices=["on", "off"], default="off",
                        help="Poll /metrics for llama-server only. Keep off for LM Studio.")
