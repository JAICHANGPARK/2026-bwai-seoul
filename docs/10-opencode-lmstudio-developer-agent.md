...
- Gemma 4를 `개발자 에이전트`로 활용해 보고 싶은 참가자
- PM이 만든 PRD를 바탕으로 코드 구현 흐름을 실습해 보고 싶은 참가자
- LM Studio에 내려받은 로컬 Gemma 4 모델을 `opencode`와 연결하고 싶은 참가자

## 먼저 결론

- `opencode + LM Studio + Gemma 4` 조합은 **조건부로 적절합니다.**
- 이유는 `opencode`가 공식적으로 **로컬 모델 + LM Studio(OpenAI-compatible endpoint)** 구성을 지원하고, 에이전트별 **프롬프트 / 모델 / 권한 / 단계 수**를 설정할 수 있기 때문입니다.
- 다만 이 조합은 **모든 참가자의 기본 실습 경로**라기보다, `개발자 에이전트`를 시연하거나 실습해 보고 싶은 참가자를 위한 **고급/선택형 경로**로 안내하는 편이 안전합니다.

왜 이렇게 보나요?

- OpenCode 공식 문서는 로컬 모델과 LM Studio 구성을 지원합니다.
- 하지만 OpenCode 공식 문서도 **코드 생성 + 툴 콜링을 모두 잘하는 모델은 많지 않다**고 설명합니다.
- 실무적으로는 `Gemma 4 E2B/E4B` 같은 소형 모델에서 코딩 에이전트 체감이 약하거나, 툴 사용 흐름이 흔들릴 수 있습니다.
- 최근 커뮤니티 이슈에서도 Gemma 4와 로컬 OpenAI-compatible 백엔드 조합에서 tool calling 실패나 루프 현상이 보고되고 있어, **100% 안정성을 전제로 한 데모**로 잡는 것은 권장하지 않습니다.

행사 운영 권장:

- 발표자 데모: `26B A4B` 또는 `31B` 권장
- 32GB 이상 참가자: 선택형으로 권장 가능
- 16GB 참가자: `E4B`로 가벼운 코딩 보조 정도만 권장
- 8GB 참가자: `opencode` 기반 개발자 에이전트 실습은 비권장

## 어떤 방식이 가장 안정적인가요?

PRD를 한 번에 길게 넣고 곧바로 전체 구현을 시키는 방식보다, 아래처럼 **2단계**로 나누는 편이 더 안정적입니다.
...
- 먼저 범위를 좁히면 tool calling과 코드 수정 정확도가 상대적으로 나아질 수 있습니다.
- 발표 흐름도 더 명확합니다.
...
즉, 이 문서의 OpenCode 설치와 실행 예시는 **PowerShell/CMD가 아니라 WSL 터미널 기준**으로 보는 편이 맞습니다.
...
- 16GB 전후: `developer.model`도 `lmstudio/google/gemma-4-e4b`로 두는 편이 안전합니다.
...
## 참가자 안내 시 꼭 같이 적어야 할 주의

- `opencode`는 이번 행사에서 **선택형 고급 경로**로 안내하는 편이 안전합니다.
- `E2B`, `E4B`는 일반 채팅보다 코딩 에이전트 체감이 약할 수 있습니다.
- 특히 작은 로컬 모델은 `tool calling`, 긴 PRD 처리, 다단계 수정 흐름에서 흔들릴 수 있습니다.
- 따라서 참가자에게는:
  - 짧은 PRD
  - 좁은 범위
  - 작은 단계
...
아래 문구는 행사 안내에 바로 넣어도 무난합니다.

```text
OpenCode는 Gemma 4를 로컬 개발자 에이전트처럼 활용해 볼 수 있는 선택형 고급 실습 경로입니다.

다만 E2B, E4B 같은 소형 모델은 Codex나 Claude Code 같은 상용 코딩 에이전트와 같은 수준의 코드 생성·도구 사용 안정성을 기대하기 어렵습니다.

가능하면 26B A4B 또는 31B 모델에서 실습하는 것을 권장하며, 작은 모델에서는 짧은 PRD와 작은 구현 범위로 진행해 주세요.
```
...
- 더 가벼운 대안으로 `pi`를 함께 안내하려면 [Pi 경량 코딩 에이전트 대안과 이번 핸즈온 선택 이유](./11-pi-and-tool-selection-notes.md) 문서를 확인해 주세요.
