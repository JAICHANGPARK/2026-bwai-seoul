...
- Apple Silicon Mac(M1/M2/M3/M4)을 쓰는 분
- LM Studio에서 GGUF 말고 MLX 방식도 이해하고 싶은 분
- MLX 기반 파생 모델이나 fine-tuned 모델이 있는지 궁금한 분
...
- 다만 이번 핸즈온 준비에서는 MLX 자체가 필수는 아닙니다.
...
- 따라서 `mlx-community` 계열 모델은 공식 Google 원본이나 Ollama, LM Studio의 기본 모델 목록과 비교해 업데이트나 패치 반영 시점이 늦을 수 있습니다.
...
- 8GB 장비로 사용하는 경우에는 작은 모델, 짧은 컨텍스트, 최소한의 앱 실행을 전제로 준비해 주세요.
