...
- MLX 파생 모델, 커뮤니티 변환 모델, 개인 fine-tuned 모델은 추가 실험용으로 보는 편이 안전합니다.

## Apple Silicon 참가자용 실무 권장안

- 하나만 설치한다면 LM Studio
- 메모리 8GB면 E2B only, best-effort
- 메모리 16GB면 E4B
- 행사 준비는 공식 기본 모델 우선
- GGUF로 준비해도 충분함
...
- 8GB 장비로 참석하는 경우에는 작은 모델, 짧은 컨텍스트, 최소한의 앱 실행을 전제로 안내하는 편이 안전합니다.
