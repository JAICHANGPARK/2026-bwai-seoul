...
- `Hermes Agent`가 정확히 어떤 도구인지 먼저 이해하고 싶은 분
- `Codex`, `Claude Code`, `OpenCode` 같은 코딩 에이전트와 무엇이 다른지 감을 잡고 싶은 분
- 왜 이번 핸즈온에서 `Hermes`를 필수 준비 항목으로 두지 않는지 알고 싶은 분
...
이번 핸즈온에서는 `Hermes`를 필수 준비 항목으로 보지 않습니다.

이유:

- Windows에서는 WSL2가 필요합니다.
- 설치와 설정할 것이 많습니다.
- 처음 준비할 때 모두가 설치하기에는 연결 과정이 복잡합니다.
...
- 처음 준비한다면 `LM Studio`만으로도 충분히 세션을 따라올 수 있습니다.
- `Hermes`는 설치뿐 아니라 모델 endpoint 연결까지 이해해야 합니다.
- Windows에서는 WSL2까지 준비해야 합니다.
...
- 에이전트 워크플로에 관심 있는 경우
- `skills`를 직접 만들어 보고 싶은 경우
- role-based workflow를 구성해 보고 싶은 경우
- 로컬 모델 위에 agent system을 얹어 보고 싶은 경우
